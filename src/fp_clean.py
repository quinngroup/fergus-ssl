...
class FergusPropagation(BaseEstimator, ClassifierMixin):
    ...
    def fit(self, X, y):
        ...
        if scipy.sparse.isspmatrix(L):
            (vals, vects) = SLA.eigsh(L, k=self.k, which='LM',
                    maxiter=1000)
        else:
            M = L.shape[0]
    ...
    def _get_kernel(self, X, y=None):
        if self.kernel == 'rbf':
            ...
            raise ValueError('%s is not a valid kernel. Only rbf and knn are supported at this time'
                              % self.kernel)
        ...
        for i in range(0, self.img_dims[1]):
            for j in range(0, self.img_dims[0]):
                if i - 1 >= 0:
                    ...
                    rbf = pairwise.rbf_kernel(X[index], X[other],
                            gamma=self.gamma)
                    A[index, other] = rbf
                    A[other, index] = rbf
                ...
                if i + 1 < self.img_dims[1] and j - 1 >= 0:
                    ...
                    A[index, other] = rbf
                    A[other, index] = rbf
                if i + 1 < self.img_dims[1] and j + 1 \
                    < self.img_dims[0]:
